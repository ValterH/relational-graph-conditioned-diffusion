...
from relgdiff.generation.diffusion import get_denoise_function
from relgdiff.generation.tabsyn.model import Model
...
def main(args):
    ...
    denoise_fn = get_denoise_function(
        in_dim, in_dim_cond, is_cond=is_cond, cond=cond_mode, device=device
    )
    ...
    for col in syn_df.columns:
        ...
        if datatype == "date":
            syn_df[col] = pd.to_datetime(
                syn_df[col].astype("int64") * 100000000000
            ).dt.date
            continue
