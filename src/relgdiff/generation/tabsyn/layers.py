...
class Encoder(nn.Module):
    def __init__(self, encoder_dim, tdim):
        super(Encoder, self).__init__()
        self.encoding_blocks = nn.ModuleList()
        for i in range(len(encoder_dim)):
            if (i + 1) == len(encoder_dim):
                break
            encoding_block = EncodingBlock(encoder_dim[i], encoder_dim[i + 1], tdim)
...
class EncodingBlock(nn.Module):
    def __init__(self, dim_in, dim_out, tdim):
        super(EncodingBlock, self).__init__()
        self.layer1 = nn.Sequential(nn.Linear(dim_in, dim_out), nn.ReLU())
        self.temb_proj = nn.Sequential(nn.Linear(tdim, dim_out), nn.ReLU())
        self.layer2 = nn.Sequential(nn.Linear(dim_out, dim_out), nn.ReLU())
...
class Decoder(nn.Module):
    def __init__(self, decoder_dim, tdim):
        super(Decoder, self).__init__()
        self.decoding_blocks = nn.ModuleList()
        for i in range(len(decoder_dim)):
            if (i + 1) == len(decoder_dim):
                break
            decoding_block = DecodingBlock(decoder_dim[i], decoder_dim[i + 1], tdim)
...
class DecodingBlock(nn.Module):
    def __init__(self, dim_in, dim_out, tdim):
        ...
        self.layer1 = nn.Sequential(nn.Linear(dim_in * 2, dim_in), nn.ReLU())
        self.temb_proj = nn.Sequential(nn.Linear(tdim, dim_in), nn.ReLU())
        self.layer2 = nn.Sequential(nn.Linear(dim_in, dim_out), nn.ReLU())
