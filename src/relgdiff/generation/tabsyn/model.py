...
from relgdiff.generation.tabsyn.diffusion_utils import EDMLoss
...
class MLPDiffusion(nn.Module):
    ...
    def forward(self, x, noise_labels, z_cond=None):
        ...
        if self.is_cond:
            x += F.silu(self.proj_cond(z_cond))
        return self.mlp(x)
    ...
    def forward(self, x, sigma, z_cond=None):
        ...
        if self.is_cond:
            F_x = self.denoise_fn_F(
                (x_in).to(dtype), c_noise.flatten(), z_cond=z_cond.to(dtype)
            )
        else:
