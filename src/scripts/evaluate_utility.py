...
def load_rossmann(method, run="1"):
    ...
    tables_synthetic = load_tables(
        f"data/synthetic/{dataset_name}/{method}/{run}/sample1", metadata
    )
    ...
    tables_test = load_tables(f'data/original/{dataset_name.split("_")[0]}/', metadata)
...
if __name__ == "__main__":
    ...
    for method in methods_to_run:
        ...
        if len(methods_to_run) < len(methods):
            with open(
                f"results/mle_{dataset_name}_{run}_{seed}_{method}.json",
                "w",
            ) as f:
                json.dump(results, f, indent=4, cls=NpEncoder)

    with open(f"results/mle_{dataset_name}_{run}_{seed}_.json", "w") as f:
        json.dump(results, f, indent=4, cls=NpEncoder)
